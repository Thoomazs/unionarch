$( function() {
    ...
    var query = Helper.getUrlVars();
    if( query.s != null && query.s.length > 0 ) {

        var $searchArea = $("#main .table tbody tr");

        $searchArea.find(".search-mark" ).replaceWith( $(this ).text() );

        $searchArea.children().each(function () {
            if($(this ).attr("class") == undefined) {
                var re = new RegExp(query.s,"gi");

                $(this).html( $(this).text().replace(re,"<span class='search-mark'>" + query.s + "</span>") );
            }
        });
    }
